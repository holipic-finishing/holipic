      ...
      <template v-for="(item, index) in items">
        ...
        >
          ...
          <v-list-tile-avatar v-if="item.title === 'Customer Paid'">
            ...
            <v-list-tile-sub-title class="text-muted caption">
            	{{ item.dated | moment("DD/MM/YYYY") }}
            </v-list-tile-sub-title>
          </v-list-tile-content>
              
          <v-list-tile-action>
            <v-list-tile-action-text class="success--text body-2" v-if="item.status == 'RECIVED'">$ +{{item.new_amount}}</v-list-tile-action-text>
