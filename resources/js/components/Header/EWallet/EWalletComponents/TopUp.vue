<template>
  <v-card class="custom-card-ewth">
    ...
    <v-list subheader two-line class="custom-list">
      ...
      <v-radio-group v-model="userChosse" hide-details>
        <v-list-tile @click="choose('visa')">
          ...
          <v-list-tile-content>
            <v-list-tile-title>
              <v-icon large color="#0157a2">fab fa-cc-visa</v-icon>
            </v-list-tile-title>
          </v-list-tile-content>
        </v-list-tile>
    ...
    <template v-if="show">
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn flat @click="$refs.form.reset()" color="blue-grey" class="white--text">Clear</v-btn>
        <v-btn
          :disabled="!form"
          :loading="isLoading"
          class="white--text"
...
export default {
  ...
  methods: {
    callWallet() {
      ...
      getWithData("e-wallet/total-ewallet", params)
        .then(res => {
          if (res.data) {
            let data = res.data;
