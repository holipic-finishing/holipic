    ...
    <v-card>
              ...
              >
                <div class="product-img mr-3">
                  ...
                  <v-tooltip bottom v-if="notification.is_read == false">
                    <v-btn slot="activator" flat icon color="#00c2e0">
                      <v-icon>fiber_manual_record</v-icon>
                    </v-btn>
                    <span>{{ $t('message.hidingRead') }}</span>
                  </v-tooltip>
                ...
                <v-list-tile-content>
                  ...
                  <span class="fs-12 grey--text text-sx-right">{{notification.created_at}}</span>
                </v-list-tile-content>
              </v-list-tile>
            </template>
          </v-list>
      ...
      <div v-else>
        <v-list two-line v-if="notifications && !notifications.length">
          <v-list-tile>
            <v-list-tile-content>
              <v-list-tile-title class="style-data">No New Notifications Found</v-list-tile-title>
            </v-list-tile-content>
          </v-list-tile>
        </v-list>
      </div>
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getWithData("notifications", params)
        .then(res => {
          if (res && res.success) {
    ...
    getAllNotification() {
      ...
      let url = "get-notifications/" + useAut.id;
      get(url)
        .then(res => {
          if (res && res.success) {
