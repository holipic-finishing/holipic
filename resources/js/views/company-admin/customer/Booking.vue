              ...
              <template slot="items" slot-scope="props">
                <td>
                  ...
                  >
                    <span
                      slot="activator"
                      @click="getDatePicker(props.item.id)"
                      style="cursor: pointer;"
                    >{{props.item.date | moment("dddd, MMMM Do YYYY")}}</span>
                    ...
                    <v-date-picker
                      ...
                      no-title
                      scrollable
                      v-if="checkDate == props.item.id"
                      @change="updateDateTime('date',props.item.id, $event)"
                    ></v-date-picker>
                  </v-dialog>
                </td>
                <td class="text-xs-left">
                  ...
                  >
                    ...
                    <v-time-picker
                      ...
                      @change="updateDateTime('time',props.item.id, $event)"
                      v-if="checkDate == props.item.id"
                    ></v-time-picker>
                  </v-dialog>
                </td>
                <td
                  class="text-xs-left"
                  v-if="props.item.photographer && props.item.photographer.branch"
                >{{ props.item.photographer.branch.name }}</td>
                <td class="text-xs-left" v-else>No Photographer</td>
                <td
                  class="text-xs-left"
                  v-if="props.item.photographer"
                >{{ props.item.photographer.name }}</td>
                <td
                  class="text-xs-left"
                  v-if="props.item.customer"
                >{{ props.item.customer.room.room_hash }}</td>
                ...
                <td class="action-width text-xs-left">
                  <v-icon small class="mr-6 hover-icon" @click="showDialog(props.item.id)">cancel</v-icon>

                  <v-icon small class="mr-6 hover-icon" @click="sendMail(props.item.id)">send</v-icon>
                </td>
              </template>
            </v-data-table>
          </v-flex>
        ...
        <v-dialog v-model="delDialog" persistent max-width="450">
          <v-card>
            <v-card-title class="headline font-weight-bold grey lighten-3">
              <v-icon large color="warning" class="mr-2">warning</v-icon>Do you want delete this item ?
            </v-card-title>
            <v-divider class="mt-0"></v-divider>
            <v-card-actions>
              <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    ...
    updateDateTime(type, id, value) {
      ...
      getWithData("booking/update/" + id, params)
        .then(response => {
    ...
    changeDate(value) {
      let params = {
        ...
        date: value,
        timezone: this.valueTimezone
      };

      getWithData("bookings", params)
        .then(response => {
          if (response && response.success) {
      ...
      get("timezones")
        .then(response => {
          if (response && response.success) {
            ...
            this.timezonesData = response.data;
            var _this = this;
            _.forEach(data, function(value, key) {
              _this.timezones.push({
                id: value.id,
                offset: value.gmt_offset + " " + value.zone_name
              });
    ...
    changeTimezone() {
      ...
      if (this.valueTimezone == "") {
        ...
        time = moment_timezone
          .tz(this.getNow(this.now), timezoneSelected["zone_name"])
          .format();
        ...
        getWithData("booking/timezone/convert", params)
          .then(response => {
            if (response && response.success) {
              this.desserts = response.data;
    ...
    getNow(now) {
      return moment(now + " " + moment().format("HH:mm:ss")).format(
        "YYYY-MM-DD HH:mm:ss"
      );
    },
    showDialog(item) {
    ...
    deleteItem() {
      del("booking/" + this.itemIdToDelete)
        .then(response => {
          if (response && response.success) {
