                    ...
                    <div class="dropdown-content">
                      ...
                      <v-list two-line>
                        <template v-for="(notification, index) in notifications">
                          <v-list-tile
                            :key="index"
                            :class="!notification.is_read ? 'style-content not-read' : 'style-content'"
                          >
                            <div class="product-img mr-3">
                              <v-tooltip bottom v-if="notification.is_read == true">
                                <v-btn slot="activator" flat icon color="#000000">
                            ...
                            <v-list-tile-content>
                              ...
                              <span
                                class="fs-12 grey--text text-sx-right"
                      ...
                      <div class="paging-bottom" v-if="notifications && notifications.length">
                        <div class="paging-bottom">
                          ...
                          <div class="pagination-style">
                            ...
                            <a
                              class="btn-nav link disabled secondary-color"
                            ...
                            <template v-if="isNotLongPage">
                              <template v-for="n in paginator.lastPage">
                                <a
                                  class="page"
                                  @click="goToPage(n)"
                                  :class="isCurrentPage(n) ? 'secondary-color' : ''"
                                >{{ n }}</a>
                              </template>
                            </template>
                            ...
                            <template v-else>
                              <template v-for="n in longPaginatorSize">
                                <a
                                  class="page"
                                  @click="goToPage(longPaginatorStart + n - 1)"
                                  :class="isCurrentPage(longPaginatorStart + n - 1) ? 'secondary-color ': ''"
                                >{{ longPaginatorStart + n - 1 }}</a>
                              </template>
                            </template>
...
export default {
  ...
  computed: {
    ...
    longPaginatorStart() {
      ...
      if (
        this.paginator.currentPage >=
        this.paginator.lastPage - this.longPaginatorEachSize
      ) {
...
.pagination-style a:hover:not(.active) {
