                      ...
                      <v-card>
                        ...
                        <v-card-actions>
                          ...
                          <v-btn
                            color="blue darken-1"
                            flat
                            @click="updateAvatarOrIdentification('avatar')"
                          >Done</v-btn>
                        </v-card-actions>
                      </v-card>
                    </v-dialog>
                      ...
                      <v-card>
                        ...
                        <v-card-actions>
                          ...
                          <v-btn
                            color="blue darken-1"
                            flat
                            @click="updateAvatarOrIdentification('identification')"
                          >Done</v-btn>
                        </v-card-actions>
                      </v-card>
                    </v-dialog>
...
export default {
  ...
  mounted() {
    this.$root.$on("showFormEditPhotgrapher", res => {
      (this.drawerRight = res.showNavigation), (this.photographer = res.data);
      if (this.photographer.status) {
        this.selectStatus = "Active";
      } else {
        this.selectStatus = "Inactive";
      }
      this.currentSelectBranch = {
  ...
  methods: {
    ...
    getBranchCompany() {
      get("company/branches?companyId=" + this.company.company_id)
        .then(response => {
    ...
    checkValue() {
      if (
        ...
        this.photographer.address == "" ||
        this.photographer.phone_number == "" ||
        this.selectStatus == ""
      ) {
        this.alertStt = true;
    ...
    editPhotographer(field, value) {
      ...
      if (this.checkValue()) {
        put("photographer/" + this.photographer.id, { params: params })
          .then(response => {
            if (response && response.success) {
              this.alertType = "success";
    ...
    updateAvatarOrIdentification(type) {
      ...
      post("photographer/" + this.photographer.id, fd)
        .then(response => {
          if (response && response.success) {
            this.alertType = "success";
            this.alertMes = response.message;
