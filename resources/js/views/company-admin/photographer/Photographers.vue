                  ...
                  <v-list two-line>
                    <template v-for="(item, index) in items">
                      <div class="hover-photographer">
                        ...
                        >
                          <v-list-tile-avatar
                            v-if="item.avatar == undefined || item.avatar == null || item.avatar == '' "
                          >
              ...
              <v-flex xs12 md7 lg8>
                <app-card>
                  <v-layout row wrap justify-space-between align-center>
                    ...
                    <v-spacer></v-spacer>
                    <div>
                      <v-btn color="indigo" class="white--text" @click="showFromAdd()">Add New</v-btn>
                    </div>
                  </v-layout>
                  ...
                  <div class="media-body" v-if="checkDetail" style="margin-top:20px">
                    <div class="media media-full">
                      <div
                        class="media-image"
                        v-if="itemDetail.avatar == null || itemDetail.avatar == ''"
                      >
                        <v-img
                            ...
                            <v-list-tile class="v-list-title--margin">
                              <v-list-tile-content>
                                <v-list-tile-title class="content-flex">
                                  ...
                                  <span
                                    class="max-value"
                                    v-if="itemDetail.branch"
                                  >{{itemDetail.branch.name}}</span>
                                </v-list-tile-title>
                              </v-list-tile-content>
                            </v-list-tile>
                            ...
                            <v-list-tile class="v-list-title--margin">
                              <v-list-tile-content>
                                <v-list-tile-title class="content-flex">
                                  ...
                                  <span
                                    class="max-value"
                                  >{{itemDetail.status ? 'Active' : 'Inactive'}}</span>
                                </v-list-tile-title>
                              </v-list-tile-content>
                            </v-list-tile>
...
export default {
  ...
  methods: {
    fetchData(activeItem) {
      get(
        "photographers?companyId=" + this.company.company_id
      ).then(response => {
    ...
    detailItem(photographerId) {
      get("photographer/detail/" + photographerId)
        .then(res => {
          if (res && res.success) {
    ...
    showInfo(item) {
      this.$root.$emit("showInfoPhototographer", {
        showNavigation: true,
        data: item
      });
    },
    ...
    showFormEdit(item) {
      this.$root.$emit("showFormEditPhotgrapher", {
        showNavigation: true,
        data: item
      });
    },
    ...
    searchPhotographer: _.debounce(function() {
      get(
          "photographers?companyId=" +
        ...
        .catch(err => {
          if (err.response && err.response && !err.response.success) {
            this.$message({
              title: "WARNING!",
