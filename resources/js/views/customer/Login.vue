      ...
      <div class="custom-box-shawdom row">
        ...
        <div class="col-md-4 right-col-login">
          <v-card tile flat>
            ...
            <v-img
              src="shop_selling/logo-holipic-new.png"
              width="100px"
            ...
            <v-card tile flat class="v-card-shop">
              <div class="custom-shop-form" style="color:black !important;">
                <v-form ref="form" v-model="valid" lazy-validation v-if="checkFormLogin">
                  <v-text-field
                    v-model="email"
                    :rules="emailRules"
                    label="Email"
                  ...
                  <v-text-field
                    ...
                    @keyup.enter="login"
                  ></v-text-field>
                  <v-btn color="primary" @click="login" class="v-btn-shop" round>Login</v-btn>
                </v-form>
                ...
                <v-form ref="form" v-model="valid" lazy-validation v-if="checkFormPassword">
                  <v-text-field
                    ...
                    @keyup.enter="login"
                  ></v-text-field>
                  <v-btn color="primary" @click="sendEmail()" class="v-btn-shop" round>Send Email</v-btn>
                </v-form>
              </div>
            </v-card>
            <v-card-title class="right-col-login--action" v-if="checkFormLogin">
              <span style="color:#b5b3b3;">
                <i class="fas fa-check-circle" style="color:#0ebff6"></i>&nbsp;Remember me
              </span>
...
export default {
  ...
  methods: {
    ...
    sendEmail() {
      if (this.$refs.form.validate()) {
        get(config.BASE_URL + "/auth/forgot-password?email=" + this.email).then(
          res => {
            if (res.data.success) {
              this.$notify({
                title: "Success",
