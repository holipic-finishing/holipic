        ...
        <v-flex xs6 class="v-flex-sp-r">
          <v-card-text class="select-languages">
            <div class="ml-4 width-select">
              <v-select :items="currencies" v-model="selectCurrency"></v-select>
            </div>

            <div class="ml-4 width-select">
              <v-select :items="languages" v-model="selectLanguage" @change="changeLanguage()"></v-select>
            </div>
          </v-card-text>
        </v-flex>
        <v-layout row wrap class="photo-selected">
          <v-flex v-for="(photo,index) in changePhotos2" xs12 sm3 d-flex :key="photo.id">
              ...
              <v-layout row v-for="type in photoTypes" :key="type.id">
                <v-flex xs3>
                  <v-card
                    ...
                    flat
                    class="cursor-v-card"
                    :class="type.active && photoOpened.size == type.size ? 'hover-v-card' : '' "
                    @click="activeClass(type)"
                  >
                    <v-card-text>{{type.size}}</v-card-text>
                  </v-card>
                </v-flex>
                <v-flex xs9 class="flex-v-number">
                  <v-list-tile>
                    ...
                    ></v-text-field>
                    <v-text-field v-else :disabled="true" single-line solo v-model="number"></v-text-field>
                    <v-btn
                      normal
                      @click="increase(photoOpened, photoOpened.quantity)"
                      v-if="photoOpened !== null && photoOpened.size == type.size "
                    >
      ...
      <div class="mb-5 ml-5 mr-5">
        <v-flex xs2 class="v-flex-sp-l">
          <v-card-text>
            ...
            <span class="ml-3 pt-3 top-relative">
              <i class="material-icons">filter</i>
            </span>
            <span class="ml-1">({{countOtherPhoto}})</span>
        ...
        <v-flex xs10 class="v-flex-sp-r line-flex"></v-flex>

        <v-layout row wrap class="photo-selected images">
          <v-flex v-for="(photo,index) in photos" xs12 sm3 d-flex :key="photo.id">
            <v-card flat tile class="d-flex custom-d-flex">
              <div class="grid-item pos-relative custom-grid-item">
                <v-img
                  :src="thumbnailDir + photo.name"
            ...
            <div class="icon-frame-photo">
              <span class="v-flex-sp-l" @click="showZoomImageAndSlide(photo,index)">
                <i class="material-icons cursor-v-card">zoom_out_map</i>
              </span>
              <span class="v-flex-sp-r">
                <i
                  class="material-icons cursor-v-card"
