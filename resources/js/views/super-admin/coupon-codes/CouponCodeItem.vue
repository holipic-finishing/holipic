<template>
  <v-card class="h-100 position-relative">
    ...
    <v-form ref="form" lazy-validation class="heigth-list-title">
      <!-- Add New Coupon -->
        ...
        <v-list-tile>
          <v-list-tile-content>
            <v-list-tile-title class="d-flex">
              ...
              <span class="contain-text-field">
                <v-switch v-model="itemToSave.active"></v-switch>
              </span>
            </v-list-tile-title>
          </v-list-tile-content>
        </v-list-tile>
              ...
              <span class="contain-text-field">
                ...
                >
                  ...
                  <v-date-picker
                    v-model="from_date"
                    no-title
                    @input="editItem('from_date', from_date)"
                  ></v-date-picker>
                </v-menu>
              </span>
            </v-list-tile-title>
...
export default {
    ...
    return {
      ...
      rules: {
        ...
        decimal: value => {
          const pattern = /^[\d]+[\.][\d]{2}$/;
          return (
            pattern.test(value) || "Please input discount format. E.g:1.00"
          );
        },
        coupon_name: value => {
  ...
  methods: {
    ...
    fetchData(field) {
      post("edit/coupon-codes/" + this.itemToLoad.id, field)
        .then(res => {
          if (res && res.success) {
  ...
  computed: {
    ...
    computedFromDateFormatted() {
      ...
      var dateFormated = this.$moment(this.itemToSave.from_date).format(
        "DD/MM/YYYY"
      );
      return dateFormated;
    },
    computedToDateFormatted() {
      ...
      var dateFormated = this.$moment(this.itemToSave.to_date).format(
        "DD/MM/YYYY"
      );
      return dateFormated;
    },
